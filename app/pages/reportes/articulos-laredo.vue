...
import type { ArticuloEstatusLogistica } from '~/types'

useHead({ title: 'Artículos en Laredo' })
...
const ESTATUS: ArticuloEstatusLogistica = 'Laredo'
...
<template>
  <UDashboardPanel id="reporte-laredo">
    <template #header>
      <UDashboardNavbar title="Artículos en Laredo">
        <template #leading>
    ...
    <template #body>
      <PrintHeader titulo="Artículos en Laredo" :subtitulo="`${filtrados.length} artículo(s) · ${fechaReporte}`" />
      ...
      <div class="mb-4 flex flex-wrap gap-3 items-center print:hidden">
        ...
        <UBadge color="neutral" variant="soft">{{ filtrados.length }} artículos · {{ totalCantidad }} pzas</UBadge>
      ...
      <div class="mb-4 grid grid-cols-3 gap-3 rounded-lg border border-default bg-elevated/30 p-3 text-sm">
        ...
        <div class="text-center">
          ...
          <p class="font-bold text-info tabular-nums">{{ fmt(totalSubtotal) }}</p>
        </div>
      </div>
          ...
          <table class="w-full border-collapse text-sm">
            ...
            <tbody>
              <tr v-if="!filtrados.length">
                <td colspan="8" class="py-12 text-center text-muted text-sm">Sin artículos en Laredo.</td>
              </tr>
              <tr v-for="a in filtrados" :key="a.id" class="hover:bg-elevated/30 transition-colors">
                ...
                <td class="px-3 py-2.5 border-b border-default text-end tabular-nums font-medium">{{ fmt(a.subtotal) }}</td>
                <td class="px-3 py-2.5 border-b border-default">
            ...
            <tfoot class="bg-elevated/50 font-semibold text-sm">
              <tr>
                ...
                <td class="px-3 py-2.5 border-t border-default text-end tabular-nums text-info">{{ fmt(totalSubtotal) }}</td>
                <td class="px-3 py-2.5 border-t border-default" />
              </tr>
            </tfoot>
          </table>
      ...
      <template v-else>
        <div v-for="g in porCliente" :key="g.cliente" class="mb-6">
          <div class="flex items-center justify-between mb-2">
            ...
            <span class="text-sm text-muted tabular-nums">{{ g.articulos.length }} arts · {{ g.totalCantidad }} pzas · {{ fmt(g.totalSubtotal) }}</span>
          </div>
          <div class="overflow-x-auto rounded-lg border border-default">
            <table class="w-full border-collapse text-sm">
              <thead>
              ...
              <tbody>
                <tr v-for="a in g.articulos" :key="a.id" class="hover:bg-elevated/30">
                  ...
                  <td class="px-3 py-2 border-b border-default text-end tabular-nums">{{ fmt(a.subtotal) }}</td>
              ...
              <tfoot class="bg-elevated/30 text-xs font-semibold">
                <tr>
                  <td colspan="4" class="px-3 py-2 border-t border-default text-muted">Subtotal cliente</td>
                  <td class="px-3 py-2 border-t border-default text-center">{{ g.totalCantidad }}</td>
                  <td class="px-3 py-2 border-t border-default text-end tabular-nums text-info">{{ fmt(g.totalSubtotal) }}</td>
